...
class SOAPRegistration
{
    ...
    public function __construct()
    {
        try
        {
            ...
            if ($this -> validateInput())
            {
                $this -> soapConnect();
                $this -> showForm = false;
                $this -> soapCommand('bnetaccount create '.$_POST["accountname"].' '.$_POST["password"]);
    ...
    protected function validateInput()
    {
        ...
        if (!isset($_POST["expansion"]) || $_POST["expansion"] != intval($_POST["expansion"]) || intval($_POST["expansion"]) < 0 || intval($_POST["expansion"]) > 6)
        {
